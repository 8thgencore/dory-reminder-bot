...
func (r *userRepository) GetByChatAndUser(ctx context.Context, chatID, userID int64) (*domain.User, error) {
	q := `SELECT id, chat_id, username, first_name, last_name, timezone, created_at, updated_at FROM users WHERE id = ? AND chat_id = ?`
	row := r.db.QueryRowContext(ctx, q, userID, chatID)
	var user domain.User
	if err := row.Scan(&user.ID, &user.ChatID, &user.Username, &user.FirstName, &user.LastName, &user.Timezone, &user.CreatedAt, &user.UpdatedAt); err != nil {
...
func (r *userRepository) Create(ctx context.Context, user *domain.User) error {
	q := `INSERT INTO users (id, chat_id, username, first_name, last_name, timezone, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
