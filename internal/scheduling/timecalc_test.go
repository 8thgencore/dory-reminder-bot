...
func TestNextWeekday(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := NextWeekday(now, tt.at, tt.weekday)
	...
	t.Run("день недели вне диапазона", func(t *testing.T) {
		_, err := NextWeekday(now, clock(9, 0), 7)
		assert.ErrorIs(t, err, ErrInvalidDate)
	})
}

func TestNextMonthDay(t *testing.T) {
	loc := time.UTC
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := NextMonthDay(tt.now, clock(9, 0), tt.day)
			require.NoError(t, err)
			assert.True(t, tt.want.Equal(got), "want %s, got %s", tt.want, got)
		})
	}
	...
	t.Run("число вне диапазона", func(t *testing.T) {
		_, err := NextMonthDay(at(loc, 2025, time.June, 1, 8, 0), clock(9, 0), 32)
		assert.ErrorIs(t, err, ErrInvalidDate)
	})
}

func TestNextYearDay(t *testing.T) {
	loc := time.UTC
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := NextYearDay(tt.now, clock(9, 0), tt.date)
			require.NoError(t, err)
			assert.True(t, tt.want.Equal(got), "want %s, got %s", tt.want, got)
		})
	}
	...
	t.Run("кривая дата возвращает ошибку, а не панику", func(t *testing.T) {
		for _, bad := range []string{"", "abc", "13", "32.13", "1.1.2025"} {
			_, err := NextYearDay(at(loc, 2025, time.June, 10, 8, 0), clock(9, 0), bad)
...
func TestAtDate(t *testing.T) {
	...
	t.Run("корректная дата", func(t *testing.T) {
		got, err := AtDate(clock(9, 0), "13.06.2025", loc)
		require.NoError(t, err)
	...
	t.Run("кривая дата возвращает ошибку", func(t *testing.T) {
		for _, bad := range []string{"", "13.06", "32.13.2025", "13/06/2025"} {
			_, err := AtDate(clock(9, 0), bad, loc)
			assert.ErrorIs(t, err, ErrInvalidDate, "input %q", bad)
		}
	})
}

func TestNextNDays(t *testing.T) {
	loc := time.UTC

	t.Run("отсчёт от даты старта", func(t *testing.T) {
		got, err := NextNDays(at(loc, 2025, time.June, 10, 0, 0), clock(9, 0), 5)
		require.NoError(t, err)
		assert.True(t, at(loc, 2025, time.June, 15, 9, 0).Equal(got))
	})

	t.Run("неположительный интервал", func(t *testing.T) {
		_, err := NextNDays(at(loc, 2025, time.June, 10, 0, 0), clock(9, 0), 0)
